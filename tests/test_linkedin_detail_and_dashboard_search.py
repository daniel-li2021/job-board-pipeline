...
class DashboardSearchTests(unittest.TestCase):
    ...
    def test_discovery_filters_are_presentation_only_for_fresh_and_rolling(self) -> None:
        ...
        self.assertIn('id="discoveryFilters"', dashboard.HTML_TEMPLATE)
        self.assertIn('id="sponsorshipFilterOptions"', dashboard.HTML_TEMPLATE)
        self.assertIn('type="checkbox" value="Sponsor"', dashboard.HTML_TEMPLATE)
        self.assertIn('type="checkbox" value="Unknown"', dashboard.HTML_TEMPLATE)
        self.assertIn('type="checkbox" value="No sponsor"', dashboard.HTML_TEMPLATE)
        ...
        self.assertIn(".discovery-filters[hidden]{display:none}", dashboard.HTML_TEMPLATE)
        job_search = dashboard.HTML_TEMPLATE.split('<div class="job-search">', 1)[1].split("</div>", 1)[0]
        self.assertIn("min-score-filter", job_search)
        self.assertIn("sponsorshipFilter", job_search)
        self.assertIn("function renderDiscoveryViews()", dashboard.HTML_TEMPLATE)
        self.assertIn("score.oninput=()=>{minScore=score.value.trim();renderDiscoveryViews()}", dashboard.HTML_TEMPLATE)
