...
- Before publishing, commit the focused task changes, fetch `origin` again, and if `main` advanced, rebase the clean task branch onto latest `origin/main` before pushing.
- After successful publication and verification, remove temporary worktrees and obsolete local task branches.
...
- Stop after the requested behavior is implemented, validated, and Git cleanup is complete.
