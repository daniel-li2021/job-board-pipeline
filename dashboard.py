...
function renderSummary(){document.getElementById('summary').innerHTML=Object.keys(names).map(k=>{const h=D.health?.components?.[k]||{status:'Warning'},keywords=(h.keywords||[]).map(value=>`<span class="pill">${esc(value)}</span>`).join('');return `<div class="card"><span>${names[k]}</span> <a class="pill health-${h.status.toLowerCase()}" target="_blank" rel="noopener noreferrer" href="${D.health_report}">${h.status}</a><div class="countline"><b>${esc(h.last_good_count??0)}</b><span>usable · updated ${esc(snapshotTime(h.last_good_at))}</span></div>${keywords?`<div class="links">${keywords}</div>`:''}<a target="_blank" rel="noopener noreferrer" href="${D.report_links[k]}">open report</a> · <a target="_blank" rel="noopener noreferrer" href="${D.health_report}">health details</a></div>`}).join('')}
