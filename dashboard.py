...
<div class="reviewbar"><span id="reviewMessage">Loading shared review status…</span></div>
...
function renderReviewMessage(){const box=document.getElementById('reviewMessage');box.classList.toggle('review-error',Boolean(sharedError));box.textContent=sharedError||(!sharedLoaded?(Object.keys(reviewStates).length?'Showing cached status while Supabase loads…':'Loading shared review status…'):'Shared status loaded · anyone with this page can edit')}
async function saveState(key,changes){if(!supabase||pendingKeys.has(key))return;const existed=Object.hasOwn(reviewStates,key),previous=reviewStates[key];const next={canonical_job_key:key,status:normalizeStatus(previous?.status||'unreviewed'),deleted:Boolean(previous?.deleted),...changes,updated_at:new Date().toISOString()};pendingKeys.add(key);reviewStates[key]=next;renderAll();const {error}=await supabase.from('job_review_status').upsert(next,{onConflict:'canonical_job_key'});pendingKeys.delete(key);if(error){if(existed)reviewStates[key]=previous;else delete reviewStates[key];sharedError=`Could not save review status: ${error.message}`}else{sharedError='';persist()}renderReviewMessage();renderAll()}
