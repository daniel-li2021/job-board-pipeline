...
.job-search{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 18px}.job-search>input[type=search]{width:min(420px,100%);min-width:240px;padding:9px 11px;border:1px solid var(--line);border-radius:10px;background:#fff;color:var(--ink)}.job-search input:focus,.filter-menu summary:focus{outline:2px solid #b9cbe3;outline-offset:1px}.job-search button{border:1px solid var(--line);border-radius:8px;padding:7px 10px;background:#fff;color:var(--ink);cursor:pointer}.discovery-filters{display:flex;align-items:center;gap:6px}.filter-field,.filter-menu summary{height:34px;display:flex;align-items:center;gap:6px;padding:5px 8px;border:1px solid var(--line);border-radius:8px;background:#fff;color:var(--muted);font-size:12px;font-weight:700}.min-score-filter{width:44px;min-width:0;padding:2px 3px;border:0;border-bottom:1px solid var(--line);background:transparent;color:var(--ink);font:inherit;font-weight:700;-moz-appearance:textfield}.min-score-filter::-webkit-outer-spin-button,.min-score-filter::-webkit-inner-spin-button{-webkit-appearance:none;margin:0}.filter-menu{position:relative}.filter-menu summary{cursor:pointer;list-style:none;color:var(--ink);white-space:nowrap}.filter-menu summary::-webkit-details-marker{display:none}.filter-menu summary::after{content:'▾';color:var(--muted);font-size:10px}.filter-menu[open] summary::after{content:'▴'}.filter-options{position:absolute;z-index:4;top:39px;left:0;min-width:155px;padding:6px;background:#fff;border:1px solid var(--line);border-radius:9px;box-shadow:0 8px 24px #1520181c}.filter-options label{display:flex;align-items:center;gap:7px;padding:6px;border-radius:6px;color:var(--ink);font-size:12px;white-space:nowrap}.filter-options label:hover{background:#f3f5f1}.export-actions{display:flex;gap:8px;margin-left:auto}
.discovery-filters[hidden]{display:none}
...
<div class="job-search"><input id="jobSearch" type="search" autocomplete="off" placeholder="Search title or company" aria-label="Search jobs by title or company"><button id="clearJobSearch" type="button" hidden>Clear</button><span id="jobSearchCount" class="small" aria-live="polite"></span><div id="discoveryFilters" class="discovery-filters"><label class="filter-field">Min score <input id="minScoreFilter" class="min-score-filter" type="number" min="0" max="100" step="1" inputmode="numeric" placeholder="Any" aria-label="Minimum score"></label><details id="sponsorshipFilter" class="filter-menu"><summary id="sponsorshipFilterLabel">Sponsorship: All</summary><div id="sponsorshipFilterOptions" class="filter-options" role="group" aria-label="Sponsorship options"><label><input type="checkbox" value="Sponsor" checked> Sponsor</label><label><input type="checkbox" value="Unknown" checked> Unknown</label><label><input type="checkbox" value="No sponsor" checked> No sponsor</label></div></details></div><div class="export-actions"><button id="copyMarkdown" type="button">Copy MD</button><button id="downloadMarkdown" type="button">Download MD</button></div></div>
...
function initializeMainViewTabs(){const buttons=[...document.querySelectorAll('#mainViewTabs [data-main-view]')];buttons.forEach((button,index)=>{button.onclick=()=>{activeMainView=button.dataset.mainView;if(activeMainView==='fresh'||activeMainView==='rolling')renderDiscoveryViews();else{renderMainViewTabs();renderSearchState();syncDiscoveryControls()}button.focus({preventScroll:true})};button.onkeydown=event=>{let target;if(event.key==='ArrowRight')target=buttons[(index+1)%buttons.length];else if(event.key==='ArrowLeft')target=buttons[(index-1+buttons.length)%buttons.length];else if(event.key==='Home')target=buttons[0];else if(event.key==='End')target=buttons[buttons.length-1];if(target){event.preventDefault();target.click()}}});renderMainViewTabs();renderSearchState();syncDiscoveryControls()}
...
function syncDiscoveryControls(){const visible=activeMainView==='fresh'||activeMainView==='rolling',filters=document.getElementById('discoveryFilters');filters.hidden=!visible;document.getElementById('minScoreFilter').value=minScore;document.querySelectorAll('#sponsorshipFilterOptions input').forEach(input=>input.checked=sponsorshipFilters.has(input.value));const selected=sponsorshipChoices.filter(value=>sponsorshipFilters.has(value));document.getElementById('sponsorshipFilterLabel').textContent=`Sponsorship: ${selected.length===sponsorshipChoices.length?'All':selected.length===1?selected[0]:selected.length+' selected'}`}
function initializeDiscoveryFilters(){const score=document.getElementById('minScoreFilter'),options=document.getElementById('sponsorshipFilterOptions');score.oninput=()=>{minScore=score.value.trim();renderDiscoveryViews()};options.onchange=event=>{if(!event.target.matches('input[type=checkbox]'))return;if(event.target.checked)sponsorshipFilters.add(event.target.value);else sponsorshipFilters.delete(event.target.value);renderDiscoveryViews()}}
...
window.addEventListener('online',refreshSharedStates);renderReviewMessage();initializeJobSearch();initializeDiscoveryFilters();initializeMarkdownExport();renderAll();initializeMainViewTabs();initializeSupabase();
