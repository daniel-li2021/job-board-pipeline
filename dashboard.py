...
from sources.schema import classify_location_bucket, normalize_company_key, normalize_sponsorship
...
<div class="job-search"><input id="jobSearch" type="search" autocomplete="off" placeholder="Search title or company" aria-label="Search jobs by title or company"><button id="clearJobSearch" type="button" hidden>Clear</button><span id="jobSearchCount" class="small" aria-live="polite"></span></div>
<div id="main-view-fresh" class="main-view" role="tabpanel" aria-labelledby="main-tab-fresh" data-main-panel="fresh"><h2>Fresh — alerted in the last 24 hours</h2><p>Jobs emitted by the latest GitHub alert Issues, including newly found A/B jobs and B→A promotions. This mirrors automation activity rather than posted_date or original first_seen.</p><p id="freshBasis" class="small"></p><div class="tabs" data-target="fresh"></div><div class="tabs company-tabs" data-company-target="fresh"></div><div id="fresh"></div></div>
...
const displayGroupKey=r=>`${r.pipeline||''}|${r.company||''}|${r.title||''}`.toLowerCase().replace(/[^a-z0-9|]+/g,' ');
function displayRows(rows){const groups=new Map();rows.forEach(r=>{const key=displayGroupKey(r);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(r)});return [...groups.values()].flatMap(group=>{const locations=new Set(group.map(r=>String(r.location||'').trim().toLowerCase()).filter(Boolean));return locations.size>1?group.map((r,index)=>({...r,_multiSize:locations.size,_multiIndex:index})):group})}
function jobs(rows,deleted=false){if(!rows.length)return '<div class="empty">No qualifying jobs in this view.</div>';return `<div class="tablewrap"><table><thead><tr><th>Tier</th><th>Company / Title</th><th>Location</th><th>Alert / Posted</th><th>Sponsorship</th><th>Referral</th><th>Status</th><th>Source</th></tr></thead><tbody>${displayRows(rows).map(r=>{const state=reviewState(r.canonical_job_key),disabled=pendingKeys.has(r.canonical_job_key)?'disabled':'',companyDisabled=pendingKeys.has(companyStateKey(r))?'disabled':'',pending=state?.pending?'<span class="small">Pending sync</span>':'',companyAction=r.company_key?` <button class="${isCompanyHidden(r)?'show-company':'hide-company'}" data-company-key="${esc(r.company_key)}" ${companyDisabled}>${isCompanyHidden(r)?'Show again':'Hide company'}</button>`:'',role=r._multiIndex?`<span class="small">↳ Same role</span><br><a href="${esc(r.url)}">${esc(r.title)}</a>`:`<b>${esc(r.company)}</b>${companyAction}<br><a href="${esc(r.url)}">${esc(r.title)}</a>${r._multiSize>1?`<div class="small">${r._multiSize} locations · individual links and states preserved</div>`:''}`;return `<tr class="${r._multiIndex?'multi-continuation':''}"><td><b>${esc(r.tier)}</b>${r.score!==''?`<div class="small">${esc(r.score)}</div>`:''}</td><td>${role}</td><td>${esc(r.location)}</td><td><span class="pill discovered">${activityText(r)}</span><div class="small">${postingText(r)}</div></td><td>${esc(r.sponsorship||'Unknown')}</td><td>${r.referral?`<span class="pill referral">${esc(r.referral)}</span>`:'-'}</td><td><div class="workflow">${deleted?`<span class="pill">Deleted</span><button class="restore" data-key="${esc(r.canonical_job_key)}" ${disabled}>Restore</button>`:`<select class="status-select" data-key="${esc(r.canonical_job_key)}" ${disabled}>${statusChoices.map(s=>`<option value="${s}" ${statusOf(r)===s?'selected':''}>${statusLabels[s]}</option>`).join('')}</select><button class="delete" data-key="${esc(r.canonical_job_key)}" ${disabled}>Delete</button>`}${pending}</div></td><td>${esc(names[r.pipeline]||r.pipeline)}</td></tr>`}).join('')}</tbody></table></div>`}
function bindStatus(box){box.querySelectorAll('.status-select').forEach(s=>s.onchange=()=>setStatus(s.dataset.key,s.value));box.querySelectorAll('.delete').forEach(b=>b.onclick=()=>deleteJob(b.dataset.key));box.querySelectorAll('.restore').forEach(b=>b.onclick=()=>restoreJob(b.dataset.key));box.querySelectorAll('.hide-company').forEach(b=>b.onclick=()=>setCompanyHidden(b.dataset.companyKey,true));box.querySelectorAll('.show-company').forEach(b=>b.onclick=()=>setCompanyHidden(b.dataset.companyKey,false))}
...
function renderSummary(){const fresh=normalRows(D.fresh_24h),rolling=normalRows(D.rolling_3d);document.getElementById('summary').innerHTML=Object.keys(names).map(k=>`<div class="card"><span>${names[k]}</span><div class="countline"><b>${fresh.filter(r=>r.pipeline===k).length}</b><span>last 24h</span><i>·</i><b>${rolling.filter(r=>r.pipeline===k).length}</b><span>in 3 days</span></div><a href="${D.report_links[k]}">open report</a></div>`).join('')}
...
function mainViewCounts(rows=uniqueRows()){return {fresh:searchedRows(normalRows(D.fresh_24h)).length,rolling:searchedRows(normalRows(D.rolling_3d)).length,'in-progress':searchedRows(rows.filter(r=>statusOf(r)==='in_progress'&&!isDeleted(r))).length,applied:searchedRows(rows.filter(r=>statusOf(r)==='applied_complete'&&!isDeleted(r))).length}}
...
function renderSearchState(rows=uniqueRows()){const clear=document.getElementById('clearJobSearch'),count=document.getElementById('jobSearchCount'),query=searchQuery.trim();clear.hidden=!query;if(!query){count.textContent='';return}const matches=mainViewCounts(rows)[activeMainView];count.textContent=`${matches} match${matches===1?'':'es'} in this view`}
function initializeMainViewTabs(){const buttons=[...document.querySelectorAll('#mainViewTabs [data-main-view]')];buttons.forEach((button,index)=>{button.onclick=()=>{activeMainView=button.dataset.mainView;renderMainViewTabs();renderSearchState();button.focus({preventScroll:true})};button.onkeydown=event=>{let target;if(event.key==='ArrowRight')target=buttons[(index+1)%buttons.length];else if(event.key==='ArrowLeft')target=buttons[(index-1+buttons.length)%buttons.length];else if(event.key==='Home')target=buttons[0];else if(event.key==='End')target=buttons[buttons.length-1];if(target){event.preventDefault();target.click()}}});renderMainViewTabs();renderSearchState()}
...
window.addEventListener('online',refreshSharedStates);renderReviewMessage();initializeJobSearch();renderAll();initializeMainViewTabs();initializeSupabase();
