...
function renderReviewMessage(){const box=document.getElementById('reviewMessage'),pending=pendingCount(),suffix=pending?` · ${pending} pending change${pending===1?'':'s'} will retry`:'';box.classList.toggle('review-error',Boolean(sharedError));box.textContent=(sharedError||(!sharedLoaded?(Object.keys(reviewStates).length?'Showing cached status while Supabase loads…':'Loading shared review status…'):'Shared status loaded · anyone with this page can edit'))+suffix}
