...
const pipelineRuns=k=>(D.health_history||[]).filter(r=>r.pipeline===k&&r.mode==='pipeline').sort((a,b)=>String(b.run_at||'').localeCompare(String(a.run_at||'')));
...
function renderSummary(){const fresh=D.counts_24h||{},rolling=D.counts_3d||{},cutoff=(Date.parse(D.generated_at)||Date.now())-86400000;document.getElementById('summary').innerHTML=Object.keys(names).map(k=>{const h=D.health?.components?.[k]||{status:'Warning'},runs=pipelineRuns(k),run=runs[0],history=k==='board'?runs.filter(item=>Date.parse(item.run_at)>=cutoff):runs.slice(0,2),runDetails=k==='board'?`<details class="small"><summary>Runs in past 24h</summary>${history.length?history.map(item=>`<div>${runYield(item)}</div>`).join(''):'<div>No runs recorded.</div>'}</details>`:`<div class="small">${history.length?history.map((item,index)=>`<div>${index?'Previous':'Latest'}: ${runYield(item)}</div>`).join(''):'No runs recorded.'}</div>`;return `<div class="card"><span>${names[k]}</span> <a class="pill health-${h.status.toLowerCase()}" target="_blank" rel="noopener noreferrer" href="${D.health_report}">${h.status}</a><div class="countline"><b>${runMetric(run,'new_jobs')}</b><span>found</span><i>·</i><b>${runMetric(run,'new_jobs_added')}</b><span>added</span><i>·</i><b>${fresh[k]||0}</b><span>last 24h</span><i>·</i><b>${rolling[k]||0}</b><span>in 3 days</span></div>${runDetails}<a target="_blank" rel="noopener noreferrer" href="${D.report_links[k]}">open report</a></div>`}).join('')}
