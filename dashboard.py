...
.job-search{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 18px}.job-search input{width:min(420px,100%);min-width:240px;padding:9px 11px;border:1px solid var(--line);border-radius:10px;background:#fff;color:var(--ink)}.job-search input:focus{outline:2px solid #b9cbe3;outline-offset:1px}.job-search button{border:1px solid var(--line);border-radius:8px;padding:7px 10px;background:#fff;color:var(--ink);cursor:pointer}.export-actions{display:flex;gap:8px;margin-left:auto}
...
function jobs(rows,deleted=false,discovery=false){if(!rows.length&&!discovery)return '<div class="empty">No qualifying jobs in this view.</div>';const tierHead=discovery?`<th class="col-tier"><span class="col-label">Tier</span><input class="min-score-filter" type="number" min="0" max="100" step="any" inputmode="decimal" placeholder="Min score" aria-label="Minimum score" value="${esc(minScore)}"></th>`:'<th>Tier</th>';const sponsorHead=discovery?`<th class="col-sponsorship"><span class="col-label">Sponsorship</span><div class="sponsorship-filters" role="group" aria-label="Sponsorship">${sponsorshipChoices.map(value=>`<button type="button" class="tab ${sponsorshipFilters.has(value)?'on':''}" data-sponsorship="${esc(value)}" aria-pressed="${String(sponsorshipFilters.has(value))}">${esc(value)}</button>`).join('')}</div></th>`:'<th>Sponsorship</th>';const body=rows.length?displayRows(rows).map(r=>{const variants=r._variants||[r],keys=variants.map(v=>v.canonical_job_key),keyData=esc(JSON.stringify(keys)),states=keys.map(reviewState),statuses=variants.map(statusOf),status=statuses.every(value=>value===statuses[0])?statuses[0]:'mixed',disabled=keys.some(key=>pendingKeys.has(key))?'disabled':'',companyDisabled=pendingKeys.has(companyStateKey(r))?'disabled':'',pending=states.some(state=>state?.pending)?'<span class="small">Pending sync</span>':'',companyAction=r.company_key?` <button class="${isCompanyHidden(r)?'show-company':'hide-company'}" data-company-key="${esc(r.company_key)}" ${companyDisabled}>${isCompanyHidden(r)?'Show again':'Hide company'}</button>`:'',location=variants.length>1?`<details><summary>${variants.length} locations</summary>${variants.map(v=>`<div><a target="_blank" rel="noopener noreferrer" href="${esc(v.url)}">${esc(v.location||'Open location')}</a></div>`).join('')}</details>`:esc(r.location),sources=[...new Set(variants.map(v=>names[v.pipeline]||v.pipeline))].join(' / ');return `<tr><td><b>${esc(r.tier)}</b>${r.score!==''?`<div class="small">${esc(r.score)}</div>`:''}</td><td><b>${esc(r.company)}</b>${companyAction}<br><a target="_blank" rel="noopener noreferrer" href="${esc(r.url)}">${esc(r.title)}</a>${variants.length>1?`<div class="small">Collapsed same role · individual location links preserved</div>`:''}</td><td>${location}</td><td><span class="pill discovered">${activityText(r)}</span><div class="small">${postingText(r)}</div></td><td>${esc(r.sponsorship||'Unknown')}</td><td>${r.referral?`<span class="pill referral">${esc(r.referral)}</span>`:'-'}</td><td><div class="workflow">${deleted?`<span class="pill">Deleted</span><button class="restore" data-keys="${keyData}" ${disabled}>Restore</button>`:`<select class="status-select" data-keys="${keyData}" ${disabled}>${status==='mixed'?'<option value="mixed" selected disabled>Mixed</option>':''}${statusChoices.map(s=>`<option value="${s}" ${status===s?'selected':''}>${statusLabels[s]}</option>`).join('')}</select><button class="delete" data-keys="${keyData}" ${disabled}>Delete</button>`}${pending}</div></td><td>${esc(sources)}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty">No qualifying jobs in this view.</div></td></tr>';return `<div class="tablewrap"><table><thead><tr>${tierHead}<th>Company / Title</th><th>Location</th><th>Alert / Posted</th>${sponsorHead}<th>Referral</th><th>Status</th><th>Source</th></tr></thead><tbody>${body}</tbody></table></div>`}
...
function initializeDiscoveryFilters(){document.addEventListener('input',event=>{if(!event.target.matches('.min-score-filter'))return;minScore=String(event.target.value||'').trim();const start=event.target.selectionStart;renderAll();const next=document.querySelector('.main-view:not([hidden]) .min-score-filter');if(!next)return;next.focus();try{if(start!=null)next.setSelectionRange(start,start)}catch(e){}});document.addEventListener('click',event=>{const button=event.target.closest('.sponsorship-filters [data-sponsorship]');if(!button)return;const value=button.dataset.sponsorship;if(sponsorshipFilters.has(value))sponsorshipFilters.delete(value);else sponsorshipFilters.add(value);renderAll()})}
