...
.job-search{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 18px}.job-search input:not([type=number]){width:min(420px,100%);min-width:240px;padding:9px 11px;border:1px solid var(--line);border-radius:10px;background:#fff;color:var(--ink)}.job-search input:not([type=number]):focus{outline:2px solid #b9cbe3;outline-offset:1px}.job-search button{border:1px solid var(--line);border-radius:8px;padding:7px 10px;background:#fff;color:var(--ink);cursor:pointer}.job-filters{display:flex;align-items:center;gap:6px;flex-wrap:wrap}.job-filters input[type=number]{width:88px;min-width:0;padding:7px 8px;border:1px solid var(--line);border-radius:8px;background:#fff;color:var(--ink)}.job-filters input[type=number]:focus{outline:2px solid #b9cbe3;outline-offset:1px}.sponsorship-filters{margin:0;gap:4px}.job-search .sponsorship-filters .tab{border:1px solid var(--line);border-radius:999px;padding:5px 9px;background:#fff;color:var(--ink);font-size:12px}.job-search .sponsorship-filters .tab.on{background:var(--ink);color:#fff}.export-actions{display:flex;gap:8px;margin-left:auto}
...
<div class="job-search"><input id="jobSearch" type="search" autocomplete="off" placeholder="Search title or company" aria-label="Search jobs by title or company"><button id="clearJobSearch" type="button" hidden>Clear</button><div id="discoveryFilters" class="job-filters" aria-label="Filter Fresh and Rolling jobs"><input id="minScoreFilter" type="number" min="0" max="100" step="any" inputmode="decimal" placeholder="Min score" aria-label="Minimum score"><div id="sponsorshipFilters" class="tabs sponsorship-filters" role="group" aria-label="Sponsorship"><button type="button" class="tab on" data-sponsorship="Sponsor" aria-pressed="true">Sponsor</button><button type="button" class="tab on" data-sponsorship="Unknown" aria-pressed="true">Unknown</button><button type="button" class="tab on" data-sponsorship="No sponsor" aria-pressed="true">No sponsor</button></div></div><span id="jobSearchCount" class="small" aria-live="polite"></span><div class="export-actions"><button id="copyMarkdown" type="button">Copy MD</button><button id="downloadMarkdown" type="button">Download MD</button></div></div>
...
function initializeDiscoveryFilters(){const input=document.getElementById('minScoreFilter'),box=document.getElementById('sponsorshipFilters');input.oninput=()=>{minScore=String(input.value||'').trim();renderAll()};box.querySelectorAll('[data-sponsorship]').forEach(button=>{button.onclick=()=>{const value=button.dataset.sponsorship;if(sponsorshipFilters.has(value))sponsorshipFilters.delete(value);else sponsorshipFilters.add(value);button.classList.toggle('on',sponsorshipFilters.has(value));button.setAttribute('aria-pressed',String(sponsorshipFilters.has(value)));renderAll()}})}
