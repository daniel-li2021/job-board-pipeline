...
<header><div><h1>Job visibility dashboard</h1><p id="updated" class="timestamp"></p><p id="sourceSnapshots" class="small"></p></div><div class="header-actions"><span id="reviewMessage" class="sync-indicator sync-expanded" aria-live="polite">● Syncing…</span><a target="_blank" rel="noopener noreferrer" id="repo">Repository</a></div></header>
...
function renderSummary(){const fresh=normalRows(D.fresh_24h),rolling=normalRows(D.rolling_3d);document.getElementById('summary').innerHTML=Object.keys(names).map(k=>`<div class="card"><span>${names[k]}</span><div class="countline"><b>${fresh.filter(r=>r.pipeline===k).length}</b><span>last 24h</span><i>·</i><b>${rolling.filter(r=>r.pipeline===k).length}</b><span>in 3 days</span></div><a target="_blank" rel="noopener noreferrer" href="${D.report_links[k]}">open report</a></div>`).join('')}
