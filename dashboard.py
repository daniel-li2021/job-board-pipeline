...
function saveStates(keys,changes){const updated_at=new Date().toISOString();keys.forEach(key=>{const previous=reviewState(key);reviewStates[key]={canonical_job_key:key,status:normalizeStatus(previous?.status||'unreviewed'),deleted:Boolean(previous?.deleted),...changes,updated_at,pending:true}});persist();renderReviewMessage();renderAll();keys.forEach(key=>pushState(reviewStates[key]))}
