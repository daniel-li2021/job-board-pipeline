...
function renderAll(){const rows=uniqueRows();renderSummary();tabs('fresh',D.fresh_24h);tabs('rolling',D.rolling_3d);renderBox('referrals',normalRows(D.referrals));renderBox('inProgress',rows.filter(r=>statusOf(r)==='in_progress'&&!isDeleted(r)));renderBox('applied',rows.filter(r=>statusOf(r)==='applied_complete'&&!isDeleted(r)));const box=document.getElementById('deleted');box.innerHTML=jobs(rows.filter(isDeleted),true);bindStatus(box);renderHiddenCompanies(rows);renderMainViewTabs(rows)}
...
window.addEventListener('online',refreshSharedStates);renderReviewMessage();renderAll();initializeMainViewTabs();initializeSupabase();
