...
def run() -> None:
    ...
    llm_errors: List[str] = []
    llm_usage = llm_config.empty_usage()
    tier1_rows: List[Dict[str, Any]] = []
    tier2_rows: List[Dict[str, Any]] = []
    enriched_rows: List[Dict[str, Any]] = []
    if use_llm:
        decisions, methods, llm_errors, llm_usage = assign_tiers(scoring_rows, swe_resume, ai_resume)
            ...
            enriched.update(
                {
                    ...
                    "reason": d["reason"],
                    "risk": d["risk"],
                    "main_gaps": "; ".join(d.get("main_gaps") or []),
    ...
    if use_llm:
        ...
        lines.append(f"- LLM usage/cost: {llm_config.format_usage(llm_usage)}")
    lines.append("")
    ...
    if use_llm:
        ...
        print(f"LLM usage/cost: {llm_config.format_usage(llm_usage)}")
        if llm_errors:
